...
ADMIN_ID = int(os.environ.get("ADMIN_ID", 1026212735))
...
async def main_menu(user_id):
    if is_admin(user_id):
        keyboard = [
            ...
            [InlineKeyboardButton("🔄 تصفير رصيد مستخدم", callback_data="admin_reset_balance_user")]
        ]
    else:
        keyboard = [
            [InlineKeyboardButton("📸 فحص صورة جديدة", callback_data="scan_new")],
...
async def setup_webhook():
    ...
    application_bot = Application.builder().token(TOKEN).updater(None).build()
    print(f"✅ Application built with token: {TOKEN[:10]}...")  # طباعة أول 10 حروف فقط للأمان

    # أضف جميع الـ handlers
    ...
    application_bot.add_handler(add_conv)

    # محادثة تصفير الرصيد (الطريقة القديمة)
    ...
    application_bot.add_handler(reset_conv)
    ...
    application_bot.add_handler(request_conv)
    ...
    application_bot.add_handler(reject_conv)
    ...
    application_bot.add_handler(edit_conv)

    # محادثة عرض تاريخ أي مستخدم
    ...
    application_bot.add_handler(admin_history_conv)

    # محادثة مسح سجل مستخدم آخر
    ...
    application_bot.add_handler(admin_clear_conv)

    application_bot.add_handler(CallbackQueryHandler(button_handler))
    application_bot.add_handler(MessageHandler(filters.PHOTO, handle_any_photo))

    # تعيين webhook
    webhook_url = f"{os.environ.get('RENDER_EXTERNAL_URL')}/webhook"
    await application_bot.bot.set_webhook(webhook_url, allowed_updates=Update.ALL_TYPES)
...
async def webhook():
    print("📨 Webhook received!")  # تأكد من وصول الطلب
    try:
        if application_bot is None:
            print("❌ ERROR: application_bot is None")
            return "Application not ready", 500
        
        json_data = request.get_json(force=True)
        print(f"📦 Update data: {json_data}")  # اطبع البيانات عشان نشوفها في الـ Logs
...
@app_flask.route('/healthcheck', methods=['GET'])
def healthcheck():
    return "OK", 200
...
if __name__ == '__main__':
    ...
    logging.info("Bot is running with webhook and Flask health check...")
    loop.run_forever()